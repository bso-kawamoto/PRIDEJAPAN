...
#if($tsstemp ne ""){	#
#試合日と開催地をGET
$sql_str4 = "SELECT * FROM $dbname WHERE contents = 34 and koukaiflag = 0 and taikaicode = $taikaicode and category = 2 and area = $area and kumi = $kumi and text1 = $num and (teamnum = $aitess or teamnum = $tss) and (linkadr = '$loginuser' or linkadr = '$aitesession' ) ORDER BY serial ASC LIMIT 1";
$rs4 = $dbh->prepare($sql_str4);
$rs4->execute();
&sqlcheck($sql_str4);
$count4 = $rs4->rows;	# Hit件数を確保
...
#END_HTML
#}
if($DPCHECK{$taikaicode}{$area}{$y}{$x} == 1){next;}
$DPCHECK{$taikaicode}{$area}{$y}{$x} = 1;
