    ...
    public function LoadDataVer1( $strFileName ){
        ...
        foreach( $lines as $value ) {
            ...
            if( isset( $value[1] ) ){
                ...
                //----------------
                switch( $value[0] ){
                    case 'page_title':       $this->mBasePageTitle        = $value[1];    continue;    //ページタイトル
                    case 'form_name':        $this->mBasicFormName        = $value[1];    continue;    //フォーム名の取得
                    case 'move_url':         $this->mBasicMoveURL         = $value[1];    continue;    //遷移先URLの取得
                    case 'mail_address':     $this->mBasicMailaddress     = $value[1];    continue;    //送信先メールアドレスの取得
                    case 'mail_subject':     $this->mBasicMailSubject     = $value[1];    continue;    //送信メールの件名
                    case 'mail_limit_time':  $this->mBasicMailLimitTime   = $value[1];    continue;    //メールの連続送信を規制する時間
                }
